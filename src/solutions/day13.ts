  ...
  return input
    .map((pattern) => {
      const n =
        ...
        getVerticalMirrorPosition(transpose2dArray(pattern), faults) * 100;
      return n;
    })
...
function getVerticalMirrorPosition(pattern: string[][], faults = 0): number {
  ...
  if (Object.keys(errorMap).length > 0) {
    return (
      Number(Object.keys(errorMap).find((key) => errorMap[key]! === faults)!) +
      1
    );
  }
  return 0;
